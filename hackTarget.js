...
const calculateHackThreads = (ns, data) => {
  const { lvl, mults, bitNodeMult, lvlReq, tName } = data;
...
const calculateGrowThreads = (ns, data) => {
  const { tGrowth, tName, tMaxMoney, mults } = data;
  ...
  const lambert =
    Math.log(var1) -
    Math.log(Math.log(var1)) -
    Math.log(1 - Math.log(Math.log(var1)) / Math.log(var1));
...
const nextStep = (ns, data) => {
  const { tName, tMinSecurity, tMaxMoney } = data;
...
const getAutoHackThreads = data => {
  ...
  const perHack =
    (((100 - tMinSecurity) * ((lvl - lvlReq + 1) / lvl)) / 24000) *
    mults.money *
    bitNodeMult;
  ...
  const lambert =
    Math.log(helper) -
    Math.log(Math.log(helper)) -
    Math.log(1 - Math.log(Math.log(helper)) / Math.log(helper));
  ...
  let weakenThreads = Math.ceil(
    (hackThreads * 0.002 + growThreads * 0.004) / 0.05
  );
...
const canAutoHack = (ns, data) => {
  const { hackThreads, growThreads, weakenThreads } = getAutoHackThreads(data);

  let totalmem = 0;
  // Add up how much memory this will use, report the value
  totalmem =
    hackThreads * 1.8 + growThreads * 1.55 + weakenThreads * 1.55 + 6.7;
...
async function doAutoHack(ns, data) {
  const { tName, tMinSecurity, host } = data;
  ...
  if (currSecurity > tMinSecurity) {
    await ns.run(
      'weaken.script',
      Math.ceil((currSecurity - tMinSecurity) / 0.05),
      tName
    );
